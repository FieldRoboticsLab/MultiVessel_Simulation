...
class RRT():
    ...
    def __init__(self,start,goal,vessel_poses_,VesselID_,yaw_):
        ...
        ship_domain_radius = self.config_data["Localization"][0]["Subscribe_to_Topics"][0]["sensing_range"]
        ...
        #NOTE: vessel_radius variable is defined in order to generate different forbidden zones depending on the lenght of the vessel
        ...
        #plotting_list_global[self.vesselID].ax.cla()
        ...
        #self.goal = (map_area*0.8,map_area*0.8)
        ...
        #This calculation will be done in self.Generate_Local_Goal_Point()
        ...
        #print(self.vessel_poses_xy)
        #print(self.vessel_poses_xy[2].vessel_id)
        #print(self.vessel_poses_xy[2].position.x)

        #We need a flag to store the data of if our graph reached the goal point or not
        self.tree_reached_to_goal_flag = False


        #This calculation is true since;
        # * In the XY conversion we use, East is positive X Axis and North is positive Y axis. 
        # * Origin point is the start point of our vessel, which is 0,0 in XY coordinates.
        ...
        #What is the first node of our graph? The starting point of the robot of course.
        self.x.append(x)
        self.y.append(y)
        ...
        self.obstacles_x = obstacle_x
        self.obstacles_y = obstacle_y
        ...
        #self.obstacles_x = [100,30,50]
        #self.obstacles_y = [100,30,100]
        
        """self.obstacle_core = vessel_radius_
        self.obstacle_radius = vessel_radius_ + vessel_safezone_"""

        #When our algorithm finishes it's work (one node of the graph reaches to goal point)
        #We will have a path
        #Our robot will follow this path in order to reach it's goal position
        self.goalstate = None
        self.path = []
        self.pathCoords = []
        ...
        #Display settings of pygame. These are for vis, therefore these are active on RRTvis.py
        #"""
        """
        pygame.display.init()
        self.MapWindowName = 'RRT Vessel'+str(self.vesselID)
        pygame.display.set_caption(self.MapWindowName)
        self.map = pygame.display.set_mode((self.map_X_max + self.map_area,self.map_Y_max + self.map_area))
        ...
        #This function will be removed after I make sure that this function works properly. 
        #I need this function for now in order to print the RRT graph, obstacles and anything 

    def Generate_Local_Path(self):
        #This function is defined to print the obstacles and path and everything via pygame.
        #self. make_random_obs_circle(3)
        #After every necessary variable is defined, it's time to start the iteration process.
    ...
    #print(len(my_rrt_object.y))
        ...
        #self.drawPath(self.getPathCoords())
    ...
    def Calculate_Local_Goal(self):
        ...
        if self.is_goal_point_free == True:
            ...
            while self.is_goal_point_free == False:
                #While we can't find any local goal point:
                ...
                #After enlarging the map, re-draw the COLREGS virtual obstacles
                #self.Generate_COLREGS_Virtual_Obstacles()
                self.Find_the_Projection_of_Global_Waypoint_to_Local_Map()
            ...
            print("Finally found a local goal.")
    ...
    def Push_Local_Goal_Clockwise(self):
        ...
        self.distance_to_initial_goal_point = 999999#placeholder to jupstart the loop.
        iteration_count = 0
...
"""
    ...
    def XY_to_LatLon(self, x_point, y_point):
        #butun hesaplamalari x y duzleminde yapiyoruz iyi guzel ancak measurement hesabinin sonucu bize x y degil
        #lat lon cinsinden lazim. Dolayisiyla kalman'dan elde edilen xy degerlerini lat lon'a cevirecek fonk bu
    ...
    def generate_random_obs_circle(self,obs_count):#generates random center point for random obstacle creation. Not used for now.
    ...
    def PathSmoothen(self):
        ...
        while(True):
            ...
            if(self.crossObstacle(smoothPath[a][0], smoothPath[b][0], smoothPath[a][1], smoothPath[b][1])):
                ...
                smoothPath.pop(b-1)
                #notpop = False
                #b won't go upper because we popped an element
    ...
    def prPathSmoothen(self):
        ...
        for node in pygameSmoothPath:
            ...
            #plotting_list_global[self.vesselID].ax.scatter(node[0],node[1],color="purple",s=self.nodeRad*2)
...
class Local_Path_Planner(threading.Thread):
    def __init__(self, VesselID_):
        ...
        #After the whole global waypoints are collected in one list, set it to a ROS parameter
            ...
            rospy.set_param('vessel'+str(vessel_id)+"_Last_Reached_Global_Wp", -1)
            rospy.set_param('vessel'+str(vessel_id)+"_Is_On_Global_Path", 1)"""
        ...
        while not rospy.is_shutdown():
            ...
            #TODO: dont forget to uncomment
                ...
                if self.generate_local_path_flag == True:
                    print("Let's generate local path for vessel",str(self.vesselID))
    ...
    def Generate_Local_Path(self):
        ...
        #def __init__(self,start,goal,vessel_poses_,VesselID_):
    ...
    def CallbackGenerateLocalPath(self,msg_):
        ...
        """
        if len(self.generate_local_path_args) == 4:
            self.generate_local_path_flag = True
        else:
            self.generate_local_path_flag = False
            """
...
#RRT Class parameters: def __init__(self,start,goal,vessel_poses_,VesselID_):



class Plotting():#!!!!!! MATPLOTLIB DOESN'T WORK WITH MULTIPLE THREADS
...
if __name__ == '__main__':
    ...
    while i <= user_defined_vessel_count:
        vessel_objects.append(Local_Path_Planner(vesselID_of_udv))
        i+=1
        vesselID_of_udv +=1
